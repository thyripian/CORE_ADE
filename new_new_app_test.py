...
- Waits for backend /health to be ready (up to ~45s).
...
- Adds /status page and passes backend diagnostics to home template.
"""
...
# Project paths (avoid ADE working-dir issues)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
BACKEND_SCRIPT = os.path.join(BASE_DIR, "run_app_dynamic.py")
...
def start_backend() -> bool:
    ...
    Ensures correct script path and PYTHONPATH so imports like database_operations.* work.
    Emits unbuffered logs to stdout/stderr for psconnect visibility.
    """
    global BACKEND_PROCESS

    if DISABLE_EMBEDDED_BACKEND:
        print("[CORE] Embedded backend startup disabled via DISABLE_EMBEDDED_BACKEND=1", flush=True)
        return True

    if BACKEND_PROCESS and BACKEND_PROCESS.poll() is None:
        print("[CORE] Backend already running", flush=True)
        return True

    if not os.path.exists(BACKEND_SCRIPT):
        print(f"[CORE] ERROR: Backend script not found: {BACKEND_SCRIPT}", flush=True)
        return False

    try:
        env = os.environ.copy()
        # Make sure the backend can import project-local packages
        env["PYTHONUNBUFFERED"] = "1"
        env["PYTHONPATH"] = BASE_DIR + (os.pathsep + env["PYTHONPATH"] if "PYTHONPATH" in env else "")
        # Avoid conflicting vars; run_app_dynamic handles db/port itself
        env.pop('DB_PATH', None)
        env.pop('API_PORT', None)

        cmd = [sys.executable, BACKEND_SCRIPT, "--port", str(BACKEND_PORT)]
        print(f"[CORE] Launching backend: {cmd}", flush=True)
        ...
        # Warm-up: wait up to ~45s, fail fast if process died
        max_attempts = 90  # 90 * 0.5s = ~45s
        for attempt in range(1, max_attempts + 1):
            # If the process exited, surface that immediately
            if BACKEND_PROCESS.poll() is not None:
                print(f"[CORE] Backend process exited early with code {BACKEND_PROCESS.returncode}", flush=True)
                return False
            try:
                r = requests.get(f"{API_BASE_URL}/health", timeout=1.5)
                if r.status_code == 200:
                    print(f"[CORE] Backend ready at {API_BASE_URL} (after {attempt*0.5:.1f}s)", flush=True)
                    return True
            except requests.exceptions.RequestException:
                pass
            time.sleep(0.5)

        print("[CORE] Backend failed to become ready within timeout", flush=True)
        return False

    except Exception as e:
        print(f"[CORE] Error starting backend: {e}", flush=True)
        return False
...
# -----------------------------
# Backend HTTP helper + diagnostics
# -----------------------------
...
def backend_diag():
    """Return a dict with child-process and HTTP health status for UI display."""
    # Process state
    if DISABLE_EMBEDDED_BACKEND:
        proc_state = "disabled"
        rc = None
    elif BACKEND_PROCESS is None:
        proc_state = "not_started"
        rc = None
    elif BACKEND_PROCESS.poll() is None:
        proc_state = "running"
        rc = None
    else:
        proc_state = "exited"
    ...
    try:
        ...
        http_payload = r.json() if http_ok else {"status_code": r.status_code}
    except Exception as e:
        http_ok = False
        http_payload = {"error": str(e)}
...
def home():
    ...
    return render_template(
        ...
        backend_diag=diag  # passed to template; harmless if unused
    )

@app.route('/status')
def status_page():
    d = backend_diag()
    # Inline HTML so you don't have to touch templates right now
    html = f"""
    <html>
      <head><title>CORE Scout Status</title></head>
      <body style="font-family:Arial,Helvetica,sans-serif; max-width: 900px; margin: 2rem auto;">
        <h1>CORE Scout — Runtime Status</h1>
        <h2>Backend Process</h2>
        <ul>
          <li>State: <b>{d['process']}</b></li>
          <li>Return code: <b>{d['return_code']}</b></li>
          <li>API Base URL: <code>{d['api_base_url']}</code></li>
        </ul>
        <h2>HTTP Health</h2>
        <ul>
          <li>Reachable: <b>{'YES' if d['http_ok'] else 'NO'}</b></li>
        </ul>
        <pre style="background:#f7f7f7; padding:1rem; border:1px solid #ddd;">{json.dumps(d['http_details'], indent=2)}</pre>
...
def api_backend_status():
    """JSON debug endpoint (optional if you can't browse to it in ADE)."""
    d = backend_diag()
    d.update({
        "script_exists": os.path.exists(BACKEND_SCRIPT),
...
def main():
    ...
    if not start_backend():
        print("[CORE] Warning: Backend failed to start. Some features may not work.", flush=True)

    print(f"[CORE] Starting Flask via Waitress on {FLASK_HOST}:{FLASK_PORT}", flush=True)
    try:
        from waitress import serve
    except ImportError:
        print("[CORE] WARNING: waitress not installed; falling back to Flask dev server", flush=True)
